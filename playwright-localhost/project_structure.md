...
// Загружаем переменные из файла .env в текущей папке
...
Например создадим файл main-page.ts, здесь мы будем хранить только локаторы (кнопки, поля).
```
// pages/main-page.ts
...
import { test, expect } from '@playwright/test';import { MainPage } from '../pages/main-page';
