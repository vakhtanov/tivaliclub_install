...
Для того чтобы заходить на сервер по SSH по ключу и выполнять команды через sudo без ввода пароля, нужно  Настройка входа по SSH без пароля (по ключу)

И Настройка sudo без пароля  

Программа sudo по умолчанию не умеет проверять SSH-ключи, поэтому для неё нужно создать исключение в конфигурации.

   1. Откройте файл настроек sudo:
  ```   
   sudo visudo
  ``` 
   2. Добавьте строку для вашего пользователя:
   ```
   username ALL=(ALL) NOPASSWD:ALL
   ```
   Замените username на ваше имя пользователя в системе.
   3. Сохраните и выйдите:
   * В nano (стандартно): Ctrl+O, Enter, Ctrl+X.
      * В vi: :wq.
